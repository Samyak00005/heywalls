...
export default function CollectionDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const byId = new Map(wallpaperData.map((row) => [row.id, mapWallpaperRow(row)]))
      const ordered = ids.map((wallpaperId) => byId.get(wallpaperId)).filter(Boolean)
  ...
  return (
    <div className="container-page pt-xl pb-3xl md:pb-4xl">
      <Link to="/collections" className="text-label text-ink-soft inline-block mb-lg">
        ← All collections
      </Link>
      <div className="flex items-end justify-between gap-lg mb-xl">
        <div>
          ...
          {collection.description && (
            <p className="text-body-sm text-ink-soft">{collection.description}</p>
          )}
        </div>
