...
export default function Categories() {
  ...
  async function saveEdit(category) {
    ...
    if (updateError) {
      setFormError(updateError.code === '23505'
        ? 'A category with that name or slug already exists.'
        : updateError.message)
      return
    }
  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-lg mb-xl">
        <div>
          <p className="text-label uppercase tracking-[0.08em] text-ink-soft mb-xs">Management</p>
          <h1 className="font-display text-h1">Categories</h1>
        </div>
        <span className="text-label text-ink-soft">{categories.length} categories</span>
      </div>
      ...
      <form onSubmit={handleAdd} className="flex gap-sm mb-2xl max-w-[640px]">
        <input
          ...
          disabled={saving}
        />
        <button type="submit" disabled={saving} className="bg-accent text-accent-contrast rounded-md px-lg py-sm text-body-sm font-medium disabled:opacity-60">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-md lg:gap-lg">
        {categories.map((c) => (
          <div key={c.id} className="bg-surface border border-line rounded-md p-lg">
            {editingId === c.id ? (
              <div className="flex gap-sm">
                <input
                  ...
                  autoFocus
                />
                <button type="button" onClick={() => saveEdit(c)} className="border border-ink rounded-sm p-sm" aria-label="Save category">
                  <Check size={15} />
                </button>
                <button type="button" onClick={cancelEdit} className="border border-line rounded-sm p-sm" aria-label="Cancel edit">
                  <X size={15} />
                </button>
              </div>
            ) : (
              <div className="flex items-center justify-between gap-lg">
                ...
                <div className="flex items-center gap-sm shrink-0">
                  <button type="button" onClick={() => beginEdit(c)} className="border border-line rounded-sm p-sm" aria-label={`Edit ${c.name}`}>
                    <Pencil size={15} />
                  </button>
                  <button type="button" disabled={deletingId === c.id} onClick={() => handleDelete(c.id)} className="border border-line text-accent-2 rounded-sm p-sm disabled:opacity-50" aria-label={`Delete ${c.name}`}>
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
