  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-lg mb-xl">
        ...
        <Link to="/admin" className="text-label text-ink-soft hover:text-ink">
          ← Dashboard
        </Link>
      </div>

      {actionError && (
        <p className="text-body-sm text-accent-2 mb-lg">{actionError}</p>
      )}
      ...
      {!loading && !error && wallpapers.length === 0 && (
        <div className="border border-line bg-surface rounded-md p-2xl">
          ...
          <p className="text-body-sm text-ink-soft">
            All uploaded wallpapers have been reviewed.
          </p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-md">
        {wallpapers.map((w) => (
          <div
            key={w.id}
            className="bg-surface border border-line rounded-md p-lg flex flex-col lg:flex-row lg:items-center gap-lg"
          >
            <Link to={`/wallpaper/${w.id}`} className="shrink-0">
              <img
                src={w.imageUrl}
                alt={w.title}
                className={`object-contain bg-bg rounded-sm border border-line ${w.orientation === "phone" ? "w-24 h-36" : "w-40 h-24"}`}
              />
            </Link>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-label text-ink-soft mt-xs">
                {w.orientation === "phone" ? "Mobile · 9:16" : "Desktop · 16:9"}
                {w.resolution ? ` · ${w.resolution}` : ""}
                {w.category ? ` · ${w.category}` : ""}
              </p>
              {w.uploader && (
                <p className="text-label text-ink-soft mt-xs">
                  Uploaded by @{w.uploader}
                </p>
              )}
            </div>
            <div className="flex flex-wrap gap-sm shrink-0">
              ...
              <button
                type="button"
                disabled={busyId === w.id}
                onClick={() => deleteWallpaper(w)}
                className="border border-line text-accent-2 rounded-md p-sm disabled:opacity-50"
