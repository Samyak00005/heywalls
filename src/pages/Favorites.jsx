...
export default function Favorites() {
  ...
  const filteredWallpapers = useMemo(
    () => applyLibraryFilters(wallpapers || [], filters),
    [wallpapers, filters],
  );
      ...
      <section className="min-w-0">
        <div className="flex flex-wrap items-end justify-between gap-lg mb-xl">
          ...
          {wallpapers.length > 0 && (
            <span className="text-label text-ink-soft">
              {filteredWallpapers.length} shown · {wallpapers.length} favourites
            </span>
          )}
        </div>
        ...
        {filteredWallpapers.length === 0 ? (
          <p className="text-body-sm text-ink-soft py-xl">
            {wallpapers.length === 0
              ? "Nothing here yet — tap the heart on any wallpaper to add it to your favourites."
