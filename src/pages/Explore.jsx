...
export default function Explore() {
  ...
  const [activeOrientation, setActiveOrientation] = useState(params.get('device') || null)
  const [sort, setSort] = useState(params.get('sort') || 'newest')
  const { categories } = useCategories()
  const { wallpapers, loading, loadingMore, error, hasMore, loadMore, totalCount } = useWallpapers({
    query,
  ...
  useEffect(() => {
    ...
    const nextUrl = next.toString() ? `/explore?${next.toString()}` : '/explore'
    ...
    <div className="container-page pt-xl pb-3xl md:pb-4xl">
      <div className="flex flex-wrap items-end justify-between gap-lg mb-xl">
        <div>
          ...
          {totalCount != null && <p className="text-body-sm text-ink-soft mt-sm">{totalCount} wallpapers found</p>}
        </div>
      </div>
        ...
        <>
          {query.trim() && wallpapers.length === 0 ? (
            <div className="py-3xl text-center">
              ...
              <p className="text-body-sm text-ink-soft mt-sm">Try a different search or browse all wallpapers.</p>
            </div>
          ) : (
