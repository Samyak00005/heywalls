...
export default function Collections() {
  ...
  async function deleteCollection(id) {
    if (!confirm('Delete this collection? The wallpapers themselves will not be deleted.')) return
  ...
  return (
    <div className="container-page pt-xl pb-3xl md:pb-4xl">
      <div className="flex items-end justify-between gap-lg mb-xl">
        ...
        <span className="text-label text-ink-soft">{collections.length} collections</span>
      </div>

      <form onSubmit={createCollection} className="border border-line bg-surface rounded-md p-lg mb-2xl max-w-[640px]">
      ...
      {collections.length === 0 ? (
        <p className="text-body-sm text-ink-soft py-xl">
          No collections yet. Create one above, then add wallpapers from their detail pages.
        </p>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-md md:gap-lg">
          {collections.map((collection) => {
            ...
            return (
              <div key={collection.id} className="border border-line bg-surface rounded-md p-lg">
                <Link to={`/collections/${collection.id}`} className="block">
                  <h2 className="font-display text-h2 mb-xs">{collection.name}</h2>
