...
export default function Profile() {
  ...
  const isOwnProfile = Boolean(
    currentProfile?.id && profile?.id === currentProfile.id,
  );
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_360px] gap-2xl lg:gap-4xl items-start">
        <section className="min-w-0">
          <section className="border-b border-line pb-2xl mb-2xl">
            <div className="flex items-start justify-between gap-lg">
              <div className="flex items-start gap-lg min-w-0">
                ...
                <div className="min-w-0 flex-1 pt-xs">
                  <h1 className="font-display text-h1 leading-tight">
                    {displayName}
                  </h1>
                  <p className="text-body-sm text-ink-soft mt-xs">
                    @{profile.username}
                  </p>
                  {profile.bio?.trim() && (
                    <p className="profile-bio text-body-sm text-ink-soft mt-sm">
                      {profile.bio.trim()}
                    </p>
                  )}
                </div>
            ...
            <div className="profile-bio-row">
              <p className="text-label text-ink-soft mt-sm">
                {uploads.length}{" "}
                {uploads.length === 1 ? "wallpaper" : "wallpapers"} published
              </p>
            </div>
          </section>
          ...
          <section className="border-t border-line pt-2xl" id="collections">
            ...
            {!isOwnProfile ? (
              <p className="text-body-sm text-ink-soft py-lg">
              ...
              <div className="profile-collection-row">
                {collections.map((collection) => {
                  const count =
                    collection.collection_wallpapers?.[0]?.count ?? 0;
                  return (
                    <Link
                      key={collection.id}
                    ...
                    >
                      <h3 className="font-display text-h3 mb-xs truncate">
                        {collection.name}
                      </h3>
          ...
          {accountError && (
            <p className="text-body-sm text-accent-2 mt-lg">{accountError}</p>
          )}
        </section>
        ...
        {isOwnProfile ? (
          <AccountSidebar
            ...
            showAdminDashboard={currentProfile?.role === "admin"}
            variant="profile"
          />
        ) : (
          <aside className="hidden lg:block border-l border-line pl-2xl sticky top-xl">
            ...
            <p className="text-body-sm text-ink-soft">
              {uploads.length} published{" "}
              {uploads.length === 1 ? "wallpaper" : "wallpapers"}.
            </p>
          </aside>
        )}
      </div>
      ...
      {deleteOpen && (
        ...
        >
          <div className="w-full max-w-[520px] border border-line bg-bg p-2xl shadow-hung">
            <div className="flex items-start justify-between gap-lg">
              <div>
                <p className="text-label uppercase tracking-[0.08em] text-accent-2 mb-xs">
                  Danger zone
                </p>
                <h2 id="delete-account-title" className="font-display text-h2">
                  Delete account
                </h2>
              </div>
              <button
            ...
            <p className="text-body-sm text-ink-soft mt-lg">
              This permanently deletes your HeyWalls account, profile, uploads,
              favourite wallpapers, and collections. This cannot be undone.
            </p>

            <form
              onSubmit={handleDeleteAccount}
              className="flex flex-col gap-lg mt-xl"
            >
              ...
              {accountError && (
                <p className="text-body-sm text-accent-2">{accountError}</p>
              )}
