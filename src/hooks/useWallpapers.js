...
async function resolveSearchIds(search) {
  ...
  const [{ data: profiles }, { data: categories }] = await Promise.all([
    supabase
      ...
      .limit(100),
    supabase.from("categories").select("id").ilike("name", pattern).limit(50),
  ]);
...
export function useWallpapers(options = {}) {
  ...
  const isDefaultCatalog =
    ...
    !categorySlug &&
    sort === "newest";
  const cached = isDefaultCatalog
    ? readWallpaperCache(CACHE_KEY, CACHE_MAX_AGE)
    : null;
  ...
  const loadBatch = useCallback(
    async (offset, append = false) => {
      ...
      const categoryFilter =
        categorySlug || (category !== "all" ? category : null);
      ...
      try {
        ...
        if (categoryFilter) {
          select =
            "id, title, description, image_url, thumbnail_url, orientation, width, height, download_count, created_at, profiles(username, display_name), wallpaper_categories!inner(category_id, categories(name, slug))";
        }
        ...
        if (orientation !== "all")
          request = request.eq("orientation", orientation);

        if (categoryFilter) {
          if (categorySlug) {
            request = request.eq(
              "wallpaper_categories.categories.slug",
              categorySlug,
            );
          } else {
            request = request.eq(
              "wallpaper_categories.categories.name",
              categoryFilter,
            );
          }
        }
