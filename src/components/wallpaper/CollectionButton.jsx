...
export default function CollectionButton({ wallpaperId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      const [{ data: collectionRows }, { data: membershipRows }] =
        await Promise.all([
          supabase
            .from("collections")
            .select("id, name")
      ...
      setSelectedIds(
        new Set((membershipRows || []).map((row) => row.collection_id)),
      );
      setLoading(false);
    }
      ...
      {open && (
        <div className="absolute z-20 top-full mt-sm right-0 w-[300px] bg-bg border border-line rounded-md shadow-hung p-lg">
          ...
          {loading && collections.length === 0 ? (
            <p className="text-label text-ink-soft py-sm">
              Loading collections…
            </p>
          ...
          <form
            ...
            className="flex gap-xs border-t border-line pt-md"
          >
            <input
              value={name}
          ...
          {message && (
            <p className="text-label text-accent-2 mt-sm">{message}</p>
          )}
        </div>
      )}
    </div>
