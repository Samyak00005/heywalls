...
export function applyLibraryFilters(items, filters) {
  ...
  const filtered = items.filter((item) => {
    const orientationMatch = filters.orientation === 'all' || item.orientation === filters.orientation
    const categoryMatch = filters.category === 'all' || (item.category || 'Uncategorized') === filters.category
    const text = `${item.title || ''} ${item.category || ''} ${item.uploader || ''}`.toLowerCase()
  ...
  return [...filtered].sort((a, b) => {
    ...
    if (filters.sort === 'downloads') return (b.downloadCount || 0) - (a.downloadCount || 0)
